from PyQt5.QtWidgets import (
    ...
    QLabel, QListWidget, QMessageBox, QHBoxLayout, QInputDialog, QDialog, QDialogButtonBox
)
...
class EditAnimalDialog(QDialog):
    def __init__(self, animal_id, animal_info, parent=None):
        ...
        self.last_weighted_input = QLineEdit()
        self.last_weighted_input.setText(animal_info.get('last_weighted', ''))
    ...
    def save_changes(self):
        ...
        last_weighted = self.last_weighted_input.text().strip()

        if not name:
            QMessageBox.warning(self, "Input Error", "Name cannot be empty.")
            return
        if not initial_weight.isdigit() or not last_weight.isdigit():
            QMessageBox.warning(self, "Input Error", "Weights must be numeric.")
            return
        ...
        self.updated_info = {
            'name': name,
            'initial_weight': int(initial_weight),
...
class AnimalsTab(QWidget):
    ...
    def add_animal(self):
        ...
        initial_weight_input.setPlaceholderText("Enter initial weight in grams")
        last_weight_input = QLineEdit()
        last_weight_input.setPlaceholderText("Enter last weight in grams")
        last_weighted_input = QLineEdit()
        last_weighted_input.setPlaceholderText("Enter last time weighted (e.g., 2024-11-05 14:30)")
        ...
        if dialog.exec_() == QDialog.Accepted:
            ...
            last_weighted = last_weighted_input.text().strip()
            ...
            animal_id = max(self.settings.get('animals', {}).keys(), default=0) + 1
    ...
    def remove_animal(self):
        ...
        animal_name = selected_item.text()
