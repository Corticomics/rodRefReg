...
class Step2SelectAnimals(QWidget):
    ...
    def __init__(self, database_handler, parent: Optional[QWidget] = None, system_controller=None):
        super().__init__(parent)
        ...
        # Get hardware limits
        self._max_cages, self._master_relay, self._valid_cages = get_available_cages(
            system_controller
        )
    ...
    def _on_selection_changed(self) -> None:
        ...
        selected = [item.data(Qt.UserRole) for item in self._animals_list.selectedItems()]
...
class Step3ConfigureParameters(QWidget):
    ...
    def __init__(
        self, database_handler=None, system_controller=None, parent: Optional[QWidget] = None
    ):
        super().__init__(parent)
        self._database_handler = database_handler
        self._system_controller = system_controller
    ...
    def _create_staggered_animal_row(self, animal: Dict[str, Any]) -> QWidget:
        ...
        # Animal label
        label = QLabel(
            f"<b>{animal.get('lab_id', animal_id)}</b> - {animal.get('name', 'Unknown')}"
        )
        ...
        start_dt.dateTimeChanged.connect(
            lambda dt, aid=animal_id: self._update_animal_config(
                aid, "start_time", dt.toPyDateTime()
            )
        )
    ...
    def _create_instant_animal_row(self, animal: Dict[str, Any]) -> QWidget:
        ...
        display_names = []
        for cage_option in self._cage_options:
            cage_combo.addItem(cage_option['display_name'], cage_option['cage_id'])
            display_names.append(cage_option['display_name'])
        ...
        delivery_dt.dateTimeChanged.connect(
            lambda dt, aid=animal_id: self._update_animal_config(
                aid, "delivery_time", dt.toPyDateTime()
            )
        )
        ...
        if animal_configs:
            ...
            for animal_id, config in animal_configs.items():
                ...
                if schedule_type == "staggered":
                    ...
                    end_str = (
                        end.strftime("%m/%d %H:%M") if isinstance(end, datetime) else str(end)
                    )
...
class ScheduleCreationWizard(QWidget):
    ...
    def _init_ui(self) -> None:
        ...
        self._step2 = Step2SelectAnimals(
            self._database_handler, system_controller=self._system_controller
        )
        ...
        self._step3 = Step3ConfigureParameters(
            database_handler=self._database_handler, system_controller=self._system_controller
        )
    ...
    def _on_step_changed(self, step_index: int) -> None:
        ...
        if step_index == 1:
            ...
            current_animals = (
                set(a["id"] for a in self._step3._selected_animals)
                if self._step3._selected_animals
                else set()
            )
