...
class UserTab(QWidget):
    ...
    def handle_login(self):
        try:
            ...
            if not username or not password:
                QMessageBox.warning(
                    self, "Input Required", "Please enter both username and password."
                )
                return
    ...
    def logout(self):
        try:
            ...
            traceback.print_exc()
            QMessageBox.critical(
                self, "Logout Error", f"An unexpected error occurred during logout:\n{str(e)}"
            )
    ...
    def handle_create_profile(self):
        ...
        def toggle_password_visibility(event):
            ...
            show_password_label.setText(
                "Hide password" if new_mode == QLineEdit.Normal else "Show password"
            )
            ...
            try:
                ...
                if success:
                    QMessageBox.information(
                        self, "Profile Created", f"Profile for '{username}' created successfully."
                    )
                else:
                    QMessageBox.warning(
                        self, "Creation Failed", f"Username '{username}' is already taken."
                    )
            except ValueError as ve:
                QMessageBox.warning(self, "Invalid Input", str(ve))
            except Exception as e:
                QMessageBox.critical(
                    self, "Profile Creation Error", f"An unexpected error occurred: {str(e)}"
                )
    ...
    def set_guest_view(self):
        ...
        try:
            ...
            QMessageBox.critical(
                self,
                "View Error",
                f"An unexpected error occurred while resetting to guest view: {str(e)}",
            )
