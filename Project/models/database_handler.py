    ...
    def get_schedule(self, schedule_id):
        ...
        try:
            with self.connect() as conn:
                ...
                # Get staggered windows if this is a staggered schedule
                if schedule_data['delivery_mode'] == 'staggered':
                    cursor.execute("""
                    ...
                    # Group windows by animal
                    window_data = {}
                    for window in windows:
                        animal_id = str(window['animal_id'])
                        if animal_id not in window_data:
                            window_data[animal_id] = []
                        window_data[animal_id].append({
                            'start_time': window['start_time'],
                            'end_time': window['end_time'],
                            'volume': window['target_volume']
                        })
                    schedule_data['window_data'] = window_data
                
                # ... rest of get_schedule implementation ...
