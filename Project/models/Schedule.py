...
class Schedule:
    ...
    def get_unit_data(self, unit_id):
        ...
        try:
            ...
            unit_animals = [
                animal_id
                for animal_id, relay_id in self.relay_unit_assignments.items()
                if relay_id == unit_id
            ]
            ...
            if self.delivery_mode == 'staggered':
                ...
                return {
                    'delivery_schedule': [
                        w for w in windows if str(w['animal_id']) in unit_animals
                    ]
                }
            else:
                unit_deliveries = [
                    d for d in self.instant_deliveries if d['relay_unit_id'] == unit_id
                ]
                return {'delivery_schedule': unit_deliveries} if unit_deliveries else None
    ...
    def get_delivery_windows(self):
        ...
        if self.delivery_mode.lower() == 'staggered':
            ...
            return [
                {'start_time': d['datetime'], 'end_time': d['datetime']}
                for d in self.instant_deliveries
            ]
