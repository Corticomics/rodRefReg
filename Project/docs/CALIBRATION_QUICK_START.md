...
**Settings → Hardware → Pulse Mode → "Round doses up to the next whole pulse"** flips that: any dose that is not an exact number of pulses gets the next whole pulse, so the plan never falls below its target and lands up to one pulse over. The rounding stays cumulative within a schedule window — a 0.6 mL window split into three chunks fires `ceil(0.6 ÷ mL/pulse)` pulses in total, not one extra per chunk.

**When to use it.** Turn it on when weighed doses come out consistently short of their target and you would rather err over than under. The bench case: with a flow-restricting needle on the reservoir, 0.3–0.7 mL doses weighed 3–8 % under target at nearest rounding; rounding up plans one more pulse per dose, which the calibration predicts brings them to +1 to +4 %. Verify with ten weighed doses at the smallest target you use — the choice is a policy, not a calibration, and only weighing tells you which side of the target your rig should sit on.

**What it does not change.** The volume per pulse, the timing profile, the per-window carry, and the logged `pulses_fired` / `volume_actual_ml` are all unaffected; only the rounding direction of the plan changes. The setting is global (all cages) and off by default.
