...
class SolenoidFlowStrategy:
    ...
    **Empirical Pulse Profiles (from valve_characterization tests):**
        ...
        *,
        ...
        if self._use_pulse_mode:
            ...
            self._empirical_pulse_volumes = {
                pw: profile.volume_mean_ml for pw, profile in calibration.pulse_profiles.items()
            }
    ...
    async def _deliver_continuous_mode(
        ...
        self._logger.info(
            f"Starting continuous delivery for cage {cage_id}: {target_volume_ml:.3f}mL ({mode_str})"
        )
        ...
        if not self._sensor_available or self._sensor is None:
            ...
            if expected_flow_ml_min > 0:
                valve_open_s = (
                    target_volume_ml / expected_flow_ml_min
            ...
            while True:
                ...
                if meas is None:
                    ...
                    if sensor_errors == 1:  # Log first sensor error
                        self._logger.warning(
                            f"Sensor read failed for cage {cage_id}, attempt {sensor_errors}/{max_sensor_errors}"
                        )
    ...
    async def _deliver_pulse_mode(
        ...
        mode_str = "WITH SENSOR" if self._sensor_available else "CALIBRATION-ONLY"
        self._logger.info(
            f"Starting pulse delivery for cage {cage_id}: {target_volume_ml:.3f}mL ({mode_str})"
        )
        ...
        if self._sensor_available and self._sensor is not None:
            ...
            if self._sensor_available and not await self._verify_sensor_health():
                self._logger.warning(
                    "Sensor health check failed, switching to CALIBRATION-ONLY mode"
                )
                self._sensor_available = False
        ...
        try:
            ...
            while delivered_ml < target_volume_ml:
                ...
                if self._sensor_available and pulses_since_restart >= max_pulses_before_restart:
                    ...
                    if not await self._restart_sensor():
                        self._logger.warning(
                            "Periodic sensor restart failed, continuing without sensor"
                        )
                        self._sensor_available = False
                    else:
                        ...
                        if not await self._verify_sensor_health():
                            self._logger.warning(
                                "Sensor health check failed after restart, continuing without sensor"
                            )
                            self._sensor_available = False
                        else:
    ...
    async def _execute_single_pulse(self, cage_id: int) -> float:
        ...
        if len(samples) >= 2:
            ...
            for i in range(1, len(samples)):
                ...
                dt_min = dt_s / 60.0
                avg_flow_segment = (
                    samples[i]['flow_ml_min'] + samples[i - 1]['flow_ml_min']
                ) / 2.0
            ...
            sample_rate_pct = (
                (len(samples) / expected_samples * 100.0) if expected_samples > 0 else 0.0
            )
        ...
        deviation_pct = (
            abs(delivered_ml - expected_vol_ml) / expected_vol_ml * 100.0
            if expected_vol_ml > 0
            else 0.0
        )
    ...
    async def _get_valve_calibration_value(self, cage_id: int) -> float:
        ...
        try:
            by_pw = (
                self._cal_snapshot.get(cage_id) if isinstance(self._cal_snapshot, dict) else None
            )
    ...
    async def _restart_sensor(self) -> bool:
        ...
        try:
            ...
            if hasattr(self._sensor, 'start'):
                ...
                await asyncio.sleep(
                    1.0
                )  # Allow firmware initialization (60ms sensor warm-up + margin)
            ...
            try:
                ...
                if meas is not None:
                    ...
                    if self._is_measurement_valid(flow_val, temp_val):
                        if attempt > 0:
                            self._logger.debug(
                                f"Sensor read successful on attempt {attempt + 1} for cage {cage_id}"
                            )
                        return meas
                    else:
                        self._logger.debug(
                            f"Invalid sensor data: flow={flow_val}, temp={temp_val}"
                        )

                # Measurement failed or invalid - check if this is a "0 bytes" case
                if attempt < max_attempts - 1:
                    self._logger.debug(
                        f"Sensor read attempt {attempt + 1} failed for cage {cage_id}, retrying..."
                    )
                    await asyncio.sleep(attempt_delay)
                    continue
