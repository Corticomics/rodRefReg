...
class ValveCalibrator:
    ...
    async def calibrate_valve(
        ...
        self.logger.info("(This will take ~{:.1f} minutes)".format(
        ...
        try:
            ...
            for i in range(num_pulses):
                ...
                if (i + 1) % 50 == 0:
                    ...
                    self.logger.info(f"  Progress: {i+1}/{num_pulses} ({progress_pct:.1f}%) - "
            ...
            while True:
                try:
                    ...
                    if measured_volume_ml > num_pulses * 0.5:  # sanity check
                        self.logger.warning(f"That seems high ({measured_volume_ml}mL for {num_pulses} pulses)")
...
Best Practices:
  ...
  - Recalibrate after valve replacement or pressure changes
    ...
    parser.add_argument('--cage', type=int, required=True,
                       help='Cage ID to calibrate (e.g., 15)')
    parser.add_argument('--relay', type=int,
                       help='Relay ID (if different from cage ID)')
    parser.add_argument('--num-pulses', type=int, default=250,
                       help='Number of pulses to execute (default: 250)')
    parser.add_argument('--pulse-width-ms', type=int, default=20,
                       help='Pulse width in milliseconds (default: 20)')
    parser.add_argument('--measured-ml', type=float,
                       help='Pre-measured volume in mL (non-interactive mode)')
    parser.add_argument('--interactive', action='store_true',
                       help='Interactive mode with user prompts')
    parser.add_argument('--trainer-id', type=int,
                       help='Trainer ID performing calibration')
    parser.add_argument('--db-path', type=str, default='rrr_database.db',
