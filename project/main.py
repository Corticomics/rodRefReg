import sys
"""
Main module for the Rodent Refreshment System.
This module initializes and runs the main application, which includes setting up the GUI,
relay handlers, notification handlers, and managing the main event loop.
Classes:
    StreamRedirector: Redirects stdout and stderr to a PyQt signal for GUI display.
Functions:
    setup(): Initializes the application settings, relay handlers, notification handlers, and GUI components.
    run_program(interval, stagger, window_start, window_end): Starts the relay control program with the specified parameters.
    cleanup(): Cleans up resources, deactivates relays, and resets the GUI state.
    stop_program(): Stops the relay control program and performs cleanup.
    create_relay_pairs(num_hats): Generates relay pairs based on the number of relay hats.
    change_relay_hats(): Prompts the user to change the number of relay hats and updates the settings and relay handler.
    main(): Entry point for the application. Initializes the QApplication, sets up the system, and starts the event loop.
Usage:
    Run this module directly to start the Rodent Refreshment System application.
"""
import os
