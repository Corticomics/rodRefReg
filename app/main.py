...
from shared.models.database import DatabaseManager  # New Import
...
def setup():
    global relay_handler, settings, gui, notification_handler, db_manager
    ...
    notification_handler = NotificationHandler(slack_token, channel_id)
    ...
    # Assuming an 'initialize' method exists; if not, remove the next line
    # notification_handler.initialize()
    
    # Initialize GUI components with db_manager
...
def run_program(interval, stagger, window_start, window_end):
    global thread, worker, notification_handler, gui
    ...
    try:
        ...
        # For example, starting any necessary threads or connections
        # Assuming an 'initialize' method exists; if not, remove the next line
        # worker_notification_handler.initialize()
        
        # Initialize RelayWorker with db_manager
        worker = RelayWorker(settings, relay_handler, worker_notification_handler, db_manager=gui.db_manager)
        ...
        # Connect worker's progress signal to the GUI's terminal
...
def cleanup():
    ...
    try:
        ...
        gui.timer = None  # Clear the timer reference, if it exists
